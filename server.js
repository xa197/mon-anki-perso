...
if (!process.env.GOOGLE_API_KEY) {
    console.error("ERREUR CRITIQUE: La variable d'environnement GOOGLE_API_KEY n'est pas définie dans le fichier .env");
    process.exit(1);
}
...
// --- ROUTE API POUR L'IA (version GEMINI CORRIGÉE) ---
app.post('/api/generate-questions', async (req, res) => {
    const { text } = req.body;
    if (!text) return res.status(400).send("Le texte est manquant.");

    try {
        // CORRECTION ICI : On utilise le nouveau nom du modèle
        ...
        console.error("--- ERREUR DÉTAILLÉE DE L'API GOOGLE ---");
        console.error(error);
        console.error("------------------------------------");
        res.status(500).send("Une erreur est survenue lors de la génération des questions par l'IA.");
    }
});
