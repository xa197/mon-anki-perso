document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ÉLÉMENTS UI (avec la variable corrigée) ---
    ...
    const libraryItemSelector = document.getElementById('library-item-selector'); // LA CORRECTION EST ICI
    ...
    libraryItemSelector.addEventListener('change', () => {
        const oldGenerateBtn = document.getElementById('generate-quiz-btn');
        if (oldGenerateBtn) oldGenerateBtn.remove();
        ...
        if (selectedItem && itemsData[selectedItem]) {
            ...
            const newGenerateBtn = document.createElement('button');
            newGenerateBtn.id = 'generate-quiz-btn';
            newGenerateBtn.className = 'ia-btn';
            newGenerateBtn.textContent = "Générer des questions par l'IA";
            newGenerateBtn.addEventListener('click', generateQuiz);
            libraryContentDisplay.insertAdjacentElement('afterend', newGenerateBtn);
        } else {
            libraryContentDisplay.innerHTML = `<p class="placeholder-text">${selectedItem ? "Aucune note enregistrée." : "Veuillez sélectionner un item."}</p>`;
        }
    });
