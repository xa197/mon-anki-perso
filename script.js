document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateQuiz(event) {
        ...
        if (!text) {
            alert("Le texte de cet item est vide. Impossible de générer un quiz.");
            return;
        }

        showView('quiz-view');
        quizTitle.textContent = `Quiz pour : ${selectedItem}`;
        quizContainer.innerHTML = ''; // Nettoyer l'ancien quiz
        ...
        try {
            const response = await fetch('/api/generate-questions', {
                ...
                body: JSON.stringify({ text: text }) // S'assurer que le texte est bien envoyé
            });
            ...
            quizContainer.innerHTML = `<p>Impossible de générer le quiz. Réessayez plus tard.</p>`;
            console.error(error);
        } finally {
