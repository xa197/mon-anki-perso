document.addEventListener('DOMContentLoaded', () => {
    ...
    const libraryItemSelector = document.getElementById('library-item-selector'), libraryContentDisplay = document.getElementById('library-content-display');
    ...
    function showNextCard() { cardContainer.classList.remove('is-flipped'); setTimeout(() => { const now = new Date(); let cardsInScope = allCards; const validItems = Array.from(itemList.options).map(o => o.value); if (currentItem && validItems.includes(currentItem)) { cardsInScope = allCards.filter(c => c.deck === currentItem); } const dueCards = cardsInScope.filter(c => new Date(c.nextReview) <= now); if (dueCards.length > 0) { currentCard = dueCards[Math.floor(Math.random() * dueCards.length)]; rectoText.textContent = currentCard.recto; versoText.textContent = currentCard.verso; rectoImage.style.display = currentCard.rectoImage ? 'block' : 'none'; rectoImage.src = currentCard.rectoImage || ''; versoImage.style.display = currentCard.versoImage ? 'block' : 'none'; versoImage.src = currentCard.versoImage || ''; showAnswerBtn.classList.remove('hidden'); difficultyButtons.classList.add('hidden'); } else { currentCard = null; rectoText.textContent = "🎉 Bravo ! Aucune carte à réviser."; versoText.textContent = ""; rectoImage.style.display = 'none'; versoImage.style.display = 'none'; showAnswerBtn.classList.add('hidden'); difficultyButtons.add('hidden'); } }, 100); }
    ...
    async function generateQuiz() {
        ...
        const text = itemsData[selectedItem];
        if (!text) return;
    
        showView('quiz-view');
        quizTitle.textContent = `Quiz pour : ${selectedItem}`;
        quizContainer.style.display = 'none'; // Cacher le conteneur du quiz
        ...
        try {
            ...
            if (!response.ok) {
                // Si le statut HTTP est une erreur (4xx, 5xx), on lève une erreur
                // avec le message d'erreur du serveur
                throw new Error(data.error || "Erreur du serveur.");
            }
            
            if (!data.questions || data.questions.length === 0) {
                throw new Error("L'IA n'a pas pu générer de questions à partir de ce texte.");
            }
            ...
            quizContainer.innerHTML = `<p style="color: var(--danger-color); text-align: center;">Impossible de générer le quiz. <br>Raison : ${error.message}</p>`;
        
        } finally {
            // **AMÉLIORATION :** Le bloc `finally` s'exécute toujours, que le `try` réussisse ou échoue.
            // C'est l'endroit parfait pour s'assurer que le spinner est caché et que le conteneur est visible.
    ...
    libraryItemSelector.addEventListener('change', () => {
        ...
        const selectedItem = libraryItemSelector.value;
        if (!selectedItem) { libraryContentDisplay.innerHTML = `<p class="placeholder-text">Veuillez sélectionner un item.</p>`; return; }
        libraryContentDisplay.textContent = itemsData[selectedItem] || '';
        if (!itemsData[selectedItem]) { libraryContentDisplay.innerHTML = `<p class="placeholder-text">Aucune note enregistrée pour cet item.</p>`; }
        const quizBtn = document.createElement('button');
        quizBtn.className = 'ia-btn';
        quizBtn.textContent = "S'entraîner (Quiz IA)";
        quizBtn.disabled = !itemsData[selectedItem];
        ...
        cardsBtn.disabled = !itemsData[selectedItem];
